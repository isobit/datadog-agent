...
var (
	...
	maxQuickGoVersionFlag = flag.String("max-quick-go", "", "max Go version to skip inspecting revision-level point releases for")
...
func main() {
	...
	var maxQuickGoVersion *goversion.GoVersion = nil
	if *maxQuickGoVersionFlag != "" {
		parsed, ok := goversion.Parse(fmt.Sprintf("go%s", *maxQuickGoVersionFlag))
		if !ok {
			log.Fatalf("unable to parse max quick Go version %q", *maxQuickGoVersionFlag)
		}
		maxQuickGoVersion = &parsed
	}
...
func run(
	...
	generator := &lutgen.LookupTableGenerator{
		...
		MaxQuickGoVersion:      maxQuickGoVersion,
		Architectures:          goArches,
		CompilationParallelism: 4,
